...
passport_1.default.use(new passport_local_1.Strategy(async (username, password, done) => {
    try {
        ...
        switch (true) {
            ...
            case !await bcryptjs_1.default.compare(password, user.password): return done(null, false, { message: 'Username or password is incorrect' });
            default: return done(null, user);
        }
    }
    catch (err) {
        return done(err);
