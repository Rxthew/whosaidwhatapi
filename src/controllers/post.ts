...
const _cascadeDeletePostComments = async function (
...
const checkUserIsAdmin = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
const checkPostOwnership = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  const post = await Post.findById({ _id: postId }).catch((error: Error) => {
    throw error;
  });
...
const confirmPostCreated = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
const confirmPostDeleted = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
const confirmPostUpdated = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  try {
    await db
      .transaction(async function finalisePostDelete(session) {
        await _cascadeDeletePostComments(req.body._id, session).catch(
          (err: Error) => {
            throw err;
          }
        );
