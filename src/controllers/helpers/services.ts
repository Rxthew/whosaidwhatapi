...
const _basicPostRequestFailed = function (
  res: Response,
...
export const basicValidation = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export const checkValidityOfPostId = function (
...
export const checkValidityOfUserId = function (
  id: string | mongoose.Types.ObjectId | unknown
) {
  const result = mongoose.isObjectIdOrHexString(id);
  const userError = () => {
...
export const checkUserIsAuthenticated = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export const postExistsInDatabase = async function (
  id: string | mongoose.Types.ObjectId | unknown
) {
  const result = await Post.exists({ _id: id }).catch((err: Error) => {
    throw err;
  });
  const postError = () => {
...
export const userExistsInDatabase = async function (
  id: string | mongoose.Types.ObjectId | unknown
) {
  const result = await User.exists({ _id: id }).catch((err: Error) => {
    throw err;
  });
  const userError = () => {
