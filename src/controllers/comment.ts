...
const _checkValidityOfCommentId = function (
...
const _commentExistsInDatabase = async function (
  id: string | mongoose.Types.ObjectId | unknown
) {
  const result = await Comment.exists({ _id: id }).catch((err: Error) => {
    throw err;
  });
  const commentError = () => {
...
const checkUserIsPrivileged = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
const checkCommentOwnership = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  const parseOwnership = async function () {
    ...
    const comment = await Comment.findById({ _id: commentId }).catch(
      (error: Error) => {
        throw error;
      }
    );
...
const confirmCommentCreated = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
const confirmCommentDeleted = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
const confirmCommentUpdated = function (
  req: Request,
  res: Response,
  next: NextFunction
) {
