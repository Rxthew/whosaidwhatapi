...
const applicableCORS = process.env.NODE_ENV === 'production' && process.env.origin ? cors({origin: process.env.origin})  :  cors()
...
app.use(function(err:HttpError, req:Request, res:Response, _next:NextFunction) {
  ...
  res.status(err.status || 500).json({'Message': `<<${err.message}>>`, 'Full Error' : `<<${err}>>`});
});
